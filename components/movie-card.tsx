...
export function MovieCard({ id, title, year, rating, image, size = "default" }: MovieCardProps) {
  const [isHovered, setIsHovered] = useState(false)
  const cardWidth = size === "large" ? "w-full" : "w-96"
  const imageHeight = size === "large" ? "h-60" : "h-56"

  return (
    <Link href={`/movie/${id}`}>
      <div
        className={`relative ${cardWidth} rounded-2xl overflow-hidden bg-gradient-to-br from-primary/40 to-secondary/20 group cursor-pointer ring-1 ring-accent/30 hover:ring-accent/60 transition-all duration-300 hover:shadow-2xl hover:shadow-accent/30 hover:scale-105`}
